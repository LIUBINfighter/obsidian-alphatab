...
export class ResourceServer {
	...
	constructor(pluginDir: string) {
		...
		console.log(
			`[AlphaTab Debug] ResourceServer initialized with directory: ${this.pluginDir}`
		);

		// 验证插件目录是否存在
		if (!fs.existsSync(this.pluginDir)) {
			console.error(
				`[AlphaTab Debug] Warning: Plugin directory does not exist: ${this.pluginDir}`
			);
		} else {
			console.log(
				`[AlphaTab Debug] Plugin directory verified: ${this.pluginDir}`
			);
			// 列出目录内容以便调试
			const contents = fs.readdirSync(this.pluginDir);
			console.log(
				`[AlphaTab Debug] Plugin directory contents:`,
				contents
			);
			...
			if (fs.existsSync(assetsDir)) {
				console.log(
					`[AlphaTab Debug] Assets directory exists: ${assetsDir}`
				);
				const assetsContents = fs.readdirSync(assetsDir);
				console.log(
					`[AlphaTab Debug] Assets contents:`,
					assetsContents
				);
				...
				if (fs.existsSync(alphatabDir)) {
					console.log(
						`[AlphaTab Debug] AlphaTab directory exists: ${alphatabDir}`
					);
					const alphatabContents = fs.readdirSync(alphatabDir);
					console.log(
						`[AlphaTab Debug] AlphaTab contents:`,
						alphatabContents
					);
					...
					if (fs.existsSync(fontDir)) {
						console.log(
							`[AlphaTab Debug] Font directory exists: ${fontDir}`
						);
						const fontContents = fs.readdirSync(fontDir);
						console.log(
							`[AlphaTab Debug] Font files:`,
							fontContents
						);
					} else {
						console.warn(
							`[AlphaTab Debug] Font directory missing: ${fontDir}`
						);
					}
				} else {
					console.warn(
						`[AlphaTab Debug] AlphaTab directory missing: ${alphatabDir}`
					);
				}
			} else {
				console.warn(
					`[AlphaTab Debug] Assets directory missing: ${assetsDir}`
				);
			}
		}
	}
		...
		return new Promise((resolve, reject) => {
			...
			this.server.listen(0, "localhost", () => {
				...
				if (address && typeof address === "object") {
					...
					console.log(
						`[AlphaTab Debug] Resource server started at ${baseUrl}`
					);
					resolve(baseUrl);
				} else {
	...
	private handleRequest(req: http.IncomingMessage, res: http.ServerResponse) {
		try {
			...
			const isAlphaTabFontRoot =
				/^assets\/alphatab\/[^/]+\.(woff2?|otf|eot|svg)$/i.test(
					relativePath
				);
			if (isAlphaTabFontRoot && !fs.existsSync(filePath)) {
				...
				if (fs.existsSync(fontFilePath)) {
					...
					console.log(
						`[AlphaTab Debug] Fallback font path: ${fontFilePath}`
					);
				}
			}
			...
			console.log(
				`[AlphaTab Debug] File exists: ${fs.existsSync(filePath)}`
			);
			...
			if (!fs.existsSync(filePath)) {
				...
				console.log(
					`[AlphaTab Debug] File not found, trying alternative path: ${alternativeFilePath}`
				);
				console.log(
					`[AlphaTab Debug] Alternative path exists: ${fs.existsSync(
						alternativeFilePath
					)}`
				);
				...
				if (fs.existsSync(alternativeFilePath)) {
					...
					// 只是为了这个请求临时使用正确的路径
					console.log(
						`[AlphaTab Debug] Using alternative path for this request`
					);
				}
			}
			...
			if (!fs.existsSync(actualFilePath)) {
				setCORSHeaders();
				console.warn(
					`[AlphaTab Debug] File not found after all attempts: ${actualFilePath}`
				);
				...
				if (relativePath.includes("font")) {
					...
					if (fontMatch) {
						console.log(
							`[AlphaTab Debug] Found matching font file: ${fontMatch}`
						);
						actualFilePath = fontMatch;
					} else {
						// 对于字体文件的优雅错误处理 - 返回较友好的响应码以防止过多错误
						console.log(
							`[AlphaTab Debug] This is a font request that failed - returning empty font`
						);
						const fontFileName = path.basename(relativePath);
						if (
							fontFileName.endsWith(".woff") ||
							fontFileName.endsWith(".woff2")
						) {
							setCORSHeaders();
							res.setHeader(
								"Content-Type",
			...
			if (
				!normalizedFilePath.startsWith(normalizedPluginDir) &&
				!normalizedFilePath.startsWith(hardcodedDir)
			) {
				setCORSHeaders();
				console.warn(
					`[AlphaTab Debug] Security: Blocked access outside plugin directory`
				);
				console.warn(
					`[AlphaTab Debug] Requested path: ${normalizedFilePath}`
				);
				res.writeHead(403);
				res.end("Forbidden");
				return;
			...
			if (stat.isDirectory()) {
				setCORSHeaders();
				console.log(
					`[AlphaTab Debug] Path is directory, not file: ${normalizedFilePath}`
				);
				res.writeHead(403);
			...
			// 设置正确的Content-Type
			let mimeType =
				mime.lookup(normalizedFilePath) || "application/octet-stream";
			...
			console.log(
				`[AlphaTab Debug] Serving file: ${normalizedFilePath} (${mimeType})`
			);
			...
			fileStream.on("error", (err) => {
				console.error(
					`[AlphaTab Debug] Error reading file ${normalizedFilePath}:`,
					err
				);
				if (!res.headersSent) {
					res.writeHead(500);
			...
			fileStream.on("end", () => {
				console.log(
					`[AlphaTab Debug] Successfully served: ${normalizedFilePath}`
				);
			});
		} catch (error) {
			// 500 错误也要加 CORS
			...
			for (const item of items) {
				...
				if (stat.isDirectory()) {
					if (item === "font" || item === "fonts") {
						console.log(
							`[AlphaTab Debug] Found font directory: ${itemPath}`
						);
					...
					item.includes("font") ||
					[".woff", ".woff2", ".eot", ".otf"].some((ext) =>
						item.endsWith(ext)
					)
				) {
					console.log(
						`[AlphaTab Debug] Found font file: ${itemPath}`
					);
				}
			}
		} catch (error) {
			console.log(
				`[AlphaTab Debug] Error searching directory ${searchDir}:`,
				error
			);
		}
	}
	...
	private configureAlphaTabSettings(): void {
		...
		console.log(
			"[AlphaTab Debug] Configured AlphaTab with resource server URLs:",
			{
	...
	private setupRoutes(): void {
		...
		this.app.use(
			"/assets",
			express.static(path.join(this.pluginDir, "assets"), {
	...
	private findMatchingFontFile(requestPath: string): string | null {
		...
		for (const fontDir of fontDirs) {
			...
			if (extensionVariants.length > 0) {
				// 优先 woff2, 其次 woff
				const woff2Match = extensionVariants.find((f) =>
					f.endsWith(".woff2")
				);
				if (woff2Match) return path.join(fontDir, woff2Match);

				const woffMatch = extensionVariants.find((f) =>
					f.endsWith(".woff")
				);
